...
void FileManager::creatFolder(string path)
{
...
bool FileManager::changeDirectoryDown(string folder)
{
    for (const auto& file : fs::directory_iterator(currentPath))
    {
...
void FileManager::creatFile(string path) //Создание файла
{
    ofstream file(path); //создать файл
    if (!file) {
